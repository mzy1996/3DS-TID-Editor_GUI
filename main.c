...
int WriteTitleIDToFile(const char* src, const char* newtid) {
    ...
    Log("Writing NCCH Header @ 0x118");
    fseek(out, 0x118, SEEK_SET);
    fwrite(tid_bytes, 1, 8, out);

    Log("Writing ExHeader @ 0x200 + 0x00");
    fseek(out, 0x200 + 0x00, SEEK_SET);
    fwrite(tid_bytes, 1, 8, out);

    Log("Writing ExHeader @ 0x200 + 0x08");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        case WM_CREATE:
            CreateWindowA("STATIC", "File:", WS_CHILD | WS_VISIBLE, 20, 20, 50, 22, hWnd, 0,0,0);
            hPath = CreateWindowA("EDIT", "", WS_CHILD | WS_BORDER | WS_VISIBLE, 70, 18, 380, 24, hWnd, (HMENU)IDC_EDIT_PATH,0,0);
            CreateWindowA("BUTTON", "Browse", WS_CHILD | WS_VISIBLE, 460,18,90,26, hWnd, (HMENU)IDC_BTN_BROWSE,0,0);

            CreateWindowA("STATIC", "Title ID:", WS_CHILD | WS_VISIBLE, 20,70,70,22, hWnd,0,0,0);
            hTID = CreateWindowA("EDIT", "", WS_CHILD | WS_BORDER | WS_VISIBLE, 90,68,280,24, hWnd, (HMENU)IDC_EDIT_TID,0,0);

            CreateWindowA("BUTTON", "PATCH & SAVE NEW FILE", WS_CHILD | WS_VISIBLE, 100,120,350,40, hWnd, (HMENU)IDC_BTN_WRITE,0,0);
            return 0;
        ...
        case WM_COMMAND:
            if (LOWORD(wp) == IDC_BTN_BROWSE) {
                ...
                if (GetOpenFileNameA(&ofn)) {
                    ...
                    char temp[256];
                    snprintf(temp, sizeof(temp), "File opened: %s", fn);
                    Log(temp);
                    ...
                    snprintf(temp, sizeof(temp), "Current TitleID: %s", tid);
                    Log(temp);
                }
            }
            ...
            if (LOWORD(wp) == IDC_BTN_WRITE) {
                if (!*g_srcPath) {
                    MessageBoxA(hWnd, "Select a file first", "Warning", MB_ICONWARNING);
                    return 0;
                }
                char tid[32];
                GetWindowTextA(hTID, tid, 32);
                if (!IsValidTID(tid)) {
                    MessageBoxA(hWnd, "TitleID must be 16 hex characters", "Error", MB_ICONERROR);
                    return 0;
                }
                WriteTitleIDToFile(g_srcPath, tid);
                MessageBoxA(hWnd, "Patch completed successfully!", "Done", MB_OK);
            }
            return 0;
